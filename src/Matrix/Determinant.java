...
public class Determinant {
    ...
    // Mengembalikan nilai determinan matriks m dengan metode reduksi baris bentuk
    // segitiga menggunakan eliminasi Gauss
    public static double determinanEliminasiGauss(Matrix mdet) {
        ...
        while (r < m.row && c < m.col) {
            int rpivot = r;
            while (rpivot < m.row - 1 && m.mat[rpivot][c] == 0) { // periksa kolom dari baris rpivot untuk kemunculan
                                                                  // elemen bukan 0 (mencari indeks pivot)
                rpivot++;
            }
